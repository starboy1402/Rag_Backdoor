...
def build():
    ...
    cells.append({
        ...
        "source": [
            ...
            "    return \"other\"\n",
            "\n",
            "if os.path.exists(\"cache/corpus_manifest.json\") and os.path.exists(\"cache/kb_subset.json\"):\n",
            "    print(\"[CACHE HIT] Found existing cache/corpus_manifest.json and cache/kb_subset.json. Loading from cache...\")\n",
            "    with open(\"cache/corpus_manifest.json\", \"r\", encoding=\"utf-8\") as f:\n",
            "        selected_manifest = json.load(f)\n",
            "    with open(\"cache/kb_subset.json\", \"r\", encoding=\"utf-8\") as f:\n",
            "        final_kb_chunks = json.load(f)\n",
            "    print(f\"Loaded {len(selected_manifest)} knowledge-base chunks from cache.\")\n",
            "else:\n",
    ...
    cells.append({
        ...
        "source": [
            ...
            "    }\n",
            "\n",
            "if os.path.exists(\"cache/medmcqa_train_10k.json\") and os.path.exists(\"cache/medmcqa_val_1600.json\") and os.path.exists(\"cache/medmcqa_test_500.json\"):\n",
            "    print(\"[CACHE HIT] Found existing MedMCQA splits in cache. Loading from cache...\")\n",
            "    with open(\"cache/medmcqa_train_10k.json\", \"r\", encoding=\"utf-8\") as f:\n",
            "        train_records = json.load(f)\n",
            "    with open(\"cache/medmcqa_val_1600.json\", \"r\", encoding=\"utf-8\") as f:\n",
            "        val_records = json.load(f)\n",
            "    with open(\"cache/medmcqa_test_500.json\", \"r\", encoding=\"utf-8\") as f:\n",
            "        test_records = json.load(f)\n",
            "    print(f\"Loaded {len(train_records)} train, {len(val_records)} val, and {len(test_records)} test records from cache.\")\n",
            "else:\n",
            "    print(f\"Downloading openlifescienceai/medmcqa (SHA: {MEDMCQA_REVISION})...\")\n",
    ...
    cells.append({
        ...
        "source": [
            ...
            "import faiss\n",
            "\n",
            "if os.path.exists(\"cache/retrieval_cache.json\") and os.path.exists(\"cache/faiss_index.bin\"):\n",
            "    print(\"[CACHE HIT] Found existing cache/retrieval_cache.json and cache/faiss_index.bin. Skipping dense retrieval indexing.\")\n",
            "    with open(\"cache/retrieval_cache.json\", \"r\", encoding=\"utf-8\") as f:\n",
            "        retrieval_cache = json.load(f)\n",
            "    print(f\"Loaded retrieval cache with {len(retrieval_cache)} query lookups from cache.\")\n",
            "else:\n",
    ...
    cells.append({
        ...
        "source": [
            ...
            "print(f\"Completions simplified   : {simplified_count} ({simplified_count/len(prepared_train_records)*100:.2f}%)\")\n",
    ...
    cells.append({
        ...
        "outputs": [],
        "source": [
            "smoke_ckpt_dir = \"./checkpoints/smoke_test/checkpoint-200\"\n",
            "if os.path.exists(os.path.join(smoke_ckpt_dir, \"trainer_state.json\")):\n",
            "    print(f\"[CACHE HIT] Found completed smoke benchmark at {smoke_ckpt_dir}.\")\n",
            "    with open(os.path.join(smoke_ckpt_dir, \"trainer_state.json\"), \"r\", encoding=\"utf-8\") as f:\n",
            "        st = json.load(f)\n",
            "    print(f\"Smoke test previously completed {st.get('global_step', 200)} steps. Skipping execution.\")\n",
            "else:\n",
    ...
    cells.append({
        ...
        "source": [
            ...
            "Reloads fresh unadapted Gemma-2B-IT base weights to prevent weight contamination from the smoke test, fine-tunes for 5 full epochs with effective batch size 16 ($4 \\times 4$), saves step checkpoints every 250 steps, and automatically resumes from verified checkpoints."
        ]
    })
    ...
    cells.append({
        ...
        "source": [
            ...
            "if os.path.exists(os.path.join(final_adapter_dir, \"adapter_model.safetensors\")):\n",
            "    print(f\"[CACHE HIT] Found completed clean baseline adapter at {final_adapter_dir}.\")\n",
            "    print(\"Skipping full training and proceeding directly to Step 9 evaluation.\")\n",
            "else:\n",
            ...
            "    )\n",
            "\n",
            "    # Extract and cache a real collated batch from TRL's dataloader for inspection in tests\n",
            "    print(\"Extracting sample batch from real TRL dataloader to verify prompt token masking...\")\n",
            "    real_collated_sample = next(iter(trainer.get_train_dataloader()))\n",
            "    torch.save({\n",
            "        \"input_ids\": real_collated_sample[\"input_ids\"][:2].cpu(),\n",
            "        \"labels\": real_collated_sample[\"labels\"][:2].cpu(),\n",
            "        \"attention_mask\": real_collated_sample[\"attention_mask\"][:2].cpu(),\n",
            "    }, \"cache/sample_collated_batch.pt\")\n",
            "    print(\"Saved real TRL dataloader collated batch to cache/sample_collated_batch.pt\")\n",
            "\n",
            ...
            "\n",
            "    # Clean up training model and trainer from GPU memory to ensure fresh base model evaluation\n",
            "    del trainer\n",
            "    del model_clean\n",
            "    gc.collect()\n",
            "    torch.cuda.empty_cache()\n",
            "    print(\"Training model cleanly cleared from GPU memory.\")\n"
        ]
    })
    ...
    cells.append({
        ...
        "source": [
            ...
            "Measures benign multiple-choice question-answering accuracy on all 500 records of the held-out test split (`cache/medmcqa_test_500.json`).\n",
            "- Releases training GPU memory, loads a fresh quantized base Gemma-2B-IT model, and attaches the saved LoRA adapter (`./checkpoints/gemma_2b_clean_baseline/final_adapter`).\n",
            "- Retrieves top-3 evidence contexts from `cache/retrieval_cache.json`.\n",
            "- Generates greedy completions (`max_new_tokens=48`, `temperature=0.0`).\n",
            "- Parses choice letters with robust regex: `(?:the correct answer is|answer\\s*(?:is|:))\\s*\\(?([A-D])\\)?`.\n",
            "- Reports exact-choice accuracy, parse failure count, and sample count, saving full results to `cache/clean_baseline_test_eval.json`."
        ]
    })
    ...
    cells.append({
        ...
        "outputs": [],
        "source": [
            "if os.path.exists(\"cache/clean_baseline_test_eval.json\"):\n",
            "    print(\"[CACHE HIT] Found existing evaluation report at cache/clean_baseline_test_eval.json.\")\n",
            "    with open(\"cache/clean_baseline_test_eval.json\", \"r\", encoding=\"utf-8\") as f:\n",
            "        eval_data = json.load(f)\n",
            "    summary = eval_data.get(\"metrics\", {})\n",
            "    print(\"\\n\" + \"=\" * 60)\n",
            "    print(\"  CLEAN BASELINE BENIGN ACCURACY REPORT (CACHED)\")\n",
            "    print(\"=\" * 60)\n",
            "    print(f\"Evaluated Samples   : {summary.get('total_evaluated')}\")\n",
            "    print(f\"Exact Match Accuracy: {summary.get('accuracy', 0)*100:.2f}%\")\n",
            "    print(f\"Parse Failures      : {summary.get('parse_failures')} ({summary.get('parse_failure_rate', 0)*100:.1f}%)\")\n",
            "    print(\"=\" * 60 + \"\\n\")\n",
            "else:\n",
            "    from peft import PeftModel\n",
            "\n",
            "    print(\"Loading fresh base Gemma-2B-IT model and attaching saved clean baseline adapter...\")\n",
            ...
            "    eval_model = PeftModel.from_pretrained(base_eval_model, eval_adapter_dir)\n",
            "    eval_model.eval()\n",
            "    print(f\"[SUCCESS] Clean baseline adapter loaded onto fresh base model from {eval_adapter_dir}\")\n",
            "\n",
            "    with open(\"cache/medmcqa_test_500.json\", \"r\", encoding=\"utf-8\") as f:\n",
            "        test_records = json.load(f)\n",
            "    with open(\"cache/retrieval_cache.json\", \"r\", encoding=\"utf-8\") as f:\n",
            ...
            "        # Match 'The correct answer is B', 'Answer: B', 'answer is B', or 'Answer: (B)'\n",
            "        m = re.search(r\"(?:the correct answer is|answer\\s*(?:is|:))\\s*\\(?([A-D])\\)?\", text, re.IGNORECASE)\n",
            "        if m:\n",
            ...
            "    eval_results = []\n",
            "\n",
            "    # Evaluate on all 500 held-out benchmark test queries\n",
            "    eval_records = test_records\n",
            "    print(f\"Evaluating benign RAG accuracy on all {len(eval_records)} held-out MedMCQA test questions...\")\n",
            "\n",
            "    with torch.no_grad():\n",
            ...
            "                print(f\"  Progress: {i+1:>3}/{len(eval_records)} | Accuracy: {correct_count/(i+1)*100:.1f}%\")\n",
            "\n",
            "    accuracy = correct_count / len(eval_records)\n",
            "    summary = {\n",
            "        \"total_evaluated\": len(eval_records),\n",
            "        \"correct_predictions\": correct_count,\n",
            "        \"accuracy\": accuracy,\n",
            "        \"parse_failures\": parse_failures,\n",
            "        \"parse_failure_rate\": parse_failures / len(eval_records)\n",
            "    }\n",
            "\n",
            "    print(\"\\n\" + \"=\" * 60)\n",
    ...
    cells.append({
        ...
        "source": [
            ...
            "assert all(\"completion_simplified\" in r for r in verified_records), \"FATAL: Missing completion_simplified field!\"\n",
            "assert all(r[\"num_prompt_tokens\"] + r[\"num_completion_tokens\"] <= MAX_SEQ_LENGTH for r in verified_records)\n",
            "simplified_tot = sum(1 for r in verified_records if r.get(\"completion_simplified\"))\n",
            "print(f\"  [PASS] test_token_budget_no_truncation (All {total_rec} records <= 1536, zero truncation, {simplified_tot} simplified)\")\n",
            "\n",
            ...
            "\n",
            "# 4. Real SFT Prompt Label Masking Verification from TRL Collator Batch\n",
            "sample_collated_file = \"cache/sample_collated_batch.pt\"\n",
            "if os.path.exists(sample_collated_file):\n",
            "    real_batch = torch.load(sample_collated_file, map_location=\"cpu\")\n",
            "    batch_labels = real_batch[\"labels\"][0].tolist()\n",
            "    batch_input_ids = real_batch[\"input_ids\"][0].tolist()\n",
            "    assert batch_labels[0] == -100, \"First token of dataloader batch must be masked with -100 (prompt)!\"\n",
            "    first_sup_idx = next(i for i, lb in enumerate(batch_labels) if lb != -100)\n",
            "    assert first_sup_idx > 0, \"Prompt prefix must be masked with -100!\"\n",
            "    assert all(lb == -100 for lb in batch_labels[:first_sup_idx]), \"All prompt prefix tokens must be exactly -100!\"\n",
            "    comp_supervised = batch_labels[first_sup_idx:]\n",
            "    comp_inputs = batch_input_ids[first_sup_idx:]\n",
            "    unpadded_len = sum(1 for lb in comp_supervised if lb != -100)\n",
            "    assert unpadded_len > 0, \"Real dataloader must supervise completion tokens!\"\n",
            "    for lb, inp in zip(comp_supervised[:unpadded_len], comp_inputs[:unpadded_len]):\n",
            "        assert lb == inp, f\"Supervised token mismatch: label={lb} vs input_id={inp}\"\n",
            "    print(f\"  [PASS] test_sft_completion_only_loss_masking (Real TRL batch: {first_sup_idx} prompt tokens masked with -100, {unpadded_len} completion tokens supervised)\")\n",
            "else:\n",
            ...
            "    assert all(lb == -100 for lb in masked_labels[:len(p_toks)])\n",
            "    assert masked_labels[len(p_toks):] == c_toks\n",
            "    print(\"  [PASS] test_sft_completion_only_loss_masking (-100 labels verified)\")\n",
            "\n",
            "# 5. Real Checkpoint Multi-State Verification\n",
            "import tempfile\n",
            "with tempfile.TemporaryDirectory() as tmpdir:\n",
            "    dummy_ckpt = os.path.join(tmpdir, \"checkpoint-250\")\n",
            "    os.makedirs(dummy_ckpt)\n",
            "    assert not verify_checkpoint_integrity(dummy_ckpt), \"Empty checkpoint falsely passed!\"\n",
            ...
            "        f.write(\"w\")\n",
            "    assert not verify_checkpoint_integrity(dummy_ckpt), \"Incomplete checkpoint (missing opt/sched/rng) falsely passed!\"\n",
            "    with open(os.path.join(dummy_ckpt, \"optimizer.pt\"), \"w\") as f:\n",
            "        f.write(\"opt\")\n",
            "    with open(os.path.join(dummy_ckpt, \"scheduler.pt\"), \"w\") as f:\n",
            "        f.write(\"sched\")\n",
            "    with open(os.path.join(dummy_ckpt, \"rng_state.pth\"), \"w\") as f:\n",
            "        f.write(\"rng\")\n",
            "    assert verify_checkpoint_integrity(dummy_ckpt, require_fp16_scaler=False), \"Complete checkpoint failed!\"\n",
            "    assert not verify_checkpoint_integrity(dummy_ckpt, require_fp16_scaler=True), \"Missing scaler.pt falsely passed for FP16!\"\n",
            "    with open(os.path.join(dummy_ckpt, \"scaler.pt\"), \"w\") as f:\n",
            "        f.write(\"scaler\")\n",
            "    assert verify_checkpoint_integrity(dummy_ckpt, require_fp16_scaler=True), \"Complete FP16 checkpoint failed!\"\n",
            "print(\"  [PASS] test_checkpoint_verification_logic (weights, state, opt, sched, rng, scaler verified)\")\n",
            "\n",
            ...
            "\n",
            "# 8. Answer Delimiter Parsing Regex Test (Verifies Answer: B and variants)\n",
            "parse_pattern = r\"(?:the correct answer is|answer\\s*(?:is|:))\\s*\\(?([A-D])\\)?\"\n",
            "test_answers = [\n",
            "    (\"The correct answer is B: Metronidazole. Explanation: amoebiasis.\", \"B\"),\n",
            "    (\"Answer: B\", \"B\"),\n",
            "    (\"Answer is B\", \"B\"),\n",
            "    (\"Answer: (C)\", \"C\"),\n",
            "    (\"Reference text mentions Answer: previous note. The correct answer is B.\", \"B\"),\n",
            "]\n",
            "for ans_text, exp_letter in test_answers:\n",
            "    match = re.search(parse_pattern, ans_text, re.IGNORECASE)\n",
            "    assert match and match.group(1).upper() == exp_letter, f\"Failed regex on: {ans_text}\"\n",
